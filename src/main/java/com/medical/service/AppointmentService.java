...
public class AppointmentService {
    ...
    @Transactional
    public Appointment createAppointment(String username, BookingRequestDto request) {
        ...
        long overlappingCount = appointmentRepository.findByDoctorIdAndAppointmentTimeBetweenOrderByAppointmentTimeAsc(
                ...
                appointmentTime.toLocalDate().atTime(LocalTime.MAX)).stream()
                .filter(a -> a.getStatus() != AppointmentStatus.CANCELED
                        && a.getAppointmentTime().isEqual(appointmentTime))
                .count();
    ...
    public List<String> getAvailableTimeSlots(Long doctorId, LocalDate date) {
        ...
        if (worksMorning) {
            long morningCount = bookedAppointments.stream()
                    .filter(a -> a.getStatus() != AppointmentStatus.CANCELED
                            && a.getAppointmentTime().toLocalTime().equals(LocalTime.of(8, 0)))
                    .count();
            if (morningCount < 10) {
        ...
        if (worksAfternoon) {
            long afternoonCount = bookedAppointments.stream()
                    .filter(a -> a.getStatus() != AppointmentStatus.CANCELED
                            && a.getAppointmentTime().toLocalTime().equals(LocalTime.of(13, 30)))
                    .count();
    ...
    private AppointmentDto mapToDto(Appointment app) {
        ...
        return AppointmentDto.builder()
                ...
                .price(priceData)
                .paymentMethod(app.getPayment() != null ? app.getPayment().getMethod().name() : "Chưa thanh toán")
                .bookedAt(app.getCreatedAt() != null
